...
class ChatConsole
{
    ...
    private function checkUpdate()
    {
        ...
        if (!$sql_update1) {
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "logs ADD ip VARCHAR (128) NOT NULL DEFAULT ''");
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "logs ADD country VARCHAR (128) NOT NULL DEFAULT ''");
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "logs ADD city VARCHAR (250) NOT NULL DEFAULT ''");
        }
        $sql_update2 = mysqli_query($this->conn,"SELECT sendLogs FROM " . $this->sql_prefix . "settings");
        if (!$sql_update2) {
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "settings ADD sendLogs BOOL NOT NULL DEFAULT 0");
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "settings ADD emailLogSubject VARCHAR(250) NOT NULL DEFAULT ''");
        ...
        if (!$sql_update3) {
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "settings ADD updated BOOL NOT NULL DEFAULT 0");
        ...
        if (mysqli_num_rows($sql_update5) == 0) {
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "settings ADD enableInitiate BOOL NOT NULL DEFAULT 0");
        }
        ...
        if (mysqli_num_rows($sql_update7) == 0) {
            mysqli_query($this->conn,"ALTER TABLE " . $this->sql_prefix . "logs ADD vcht_id INT(11) NOT NULL DEFAULT 0");
    ...
    private function checkInstallation()
    {
        ...
        if($rep != 1) {
            ...
            error_log('INIT: settings table not found, now creating table');
            ...
            `bounceFx` tinyint(1) NOT NULL DEFAULT 0,
            `updated` tinyint(1) NOT NULL DEFAULT 0,
            `playSound` tinyint(1) NOT NULL DEFAULT 0,
            `url` varchar(250) NOT NULL DEFAULT '',
            `chatPosition` varchar(8) NOT NULL DEFAULT '',
            `sendLogs` tinyint(1) NOT NULL DEFAULT 0,
            `emailLogSubject` varchar(250) NOT NULL DEFAULT '',
            `enableInitiate` tinyint(1) NOT NULL DEFAULT 0,
	     UNIQUE KEY id (id)
            ...
            error_log('DONE: settings table Created');
	     ...
	     UNIQUE KEY id (id)
            ...
            error_log('DONE: logs table Created');
	     ...
	     UNIQUE KEY id (id)
            ...
            error_log('DONE: messages table Created');
		...
		UNIQUE KEY id (id)
            ...
            error_log('DONE: operators table Created');
		...
		UNIQUE KEY id (id)
            ...
            error_log('DONE: users table Created');
		...
		UNIQUE KEY id (id)
            ...
            error_log('DONE: texts table Created');

        // Support Tickets table
            $db_table_name = $this->sql_prefix . 'tickets';
            $sql = "CREATE TABLE IF NOT EXISTS `$db_table_name` (
            `id` mediumint(9) NOT NULL AUTO_INCREMENT,
            `date` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
            `lastActivity` datetime DEFAULT NULL,
            ...
            `sent` tinyint(1) NOT NULL DEFAULT 0,
            `transfer` tinyint(1) NOT NULL DEFAULT 0,
            `echat_id` int(11) NOT NULL DEFAULT 0,
         UNIQUE KEY id (id)
          ) ENGINE=MyISAM AUTO_INCREMENT=31 DEFAULT CHARSET=utf8;";
            mysqli_query($this->conn, $sql)or die(mysqli_error($this->conn));

            error_log('DONE: tickets table created...');
