...
function recoverCountries($sql_prefix)
{
    $sql_logs = mysqli_query($connection,'SELECT * FROM ' . $sql_prefix . 'logs WHERE country=""') or die(mysqli_error($connection));
    while ($log = mysqli_fetch_object($sql_logs)) {
        try {
            ...
            mysqli_query($connection,'UPDATE ' . $sql_prefix . 'logs SET country="' . $country . '", city="' . $city . '" WHERE id=' . $log->id) or die(mysqli_error($connection));
        }catch (Throwable $t) {           
            echo 'no';
          } catch (Exception $e) {
            echo 'no';
...
<?php
    ...
    if ($_POST['action'] == 'vcht_recoverPass') {
        ...
        if (mysqli_num_rows($sql_operator) > 0) {
            ...
            $headers .= "Mime-Version: 1.0\n";
            $headers .= "Content-type: text/html; charset=utf-8\n";
            $urlConsole = $settings->url . 'chat-app/console/';
