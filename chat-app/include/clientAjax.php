...
//$bdd = mysql_select_db($sql_database_name, $connection) or die('Configuration Error : Can\'t select the database');
...
/*
function table_exists($tables) {
  $eror = false;
  // if $tables is a string, convert it into an Array
  if(is_string($tables)) $tables = array($tables);

  $result=$connection->query("SHOW TABLES IN `$sql_database_name`");
  if($result !== false) {
    if($result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
        $tables_db[] = $row['Tables_in_'.$sql_database_name];
      }

      for($i=0; $i<count($tables); $i++) {
        if(!in_array($tables[$i], $tables_db)) $eror[] = 'Table '. $tables[$i]. ' Not exist in "'. $sql_database_name. '"';
      }
    }
    else $eror[] = 'There is no table in "'. $sql_database_name. '"';
  }
  else $eror[] = 'Unable to check the "'. $sql_database_name. '"';
    ...
    if ($_POST['action'] == 'vcht_newMessage') {
        ...
        if ($logID == 0) {
            ...
            if (mysqli_num_rows($sql_logs) > 0) {
                ...
                mysqli_query($connection,'INSERT INTO ' . $sql_prefix . 'logs (username,operatorID,date,ip) VALUES ("' . $username . '","' . $operatorID . '","' . date('Y-m-d h:i:s') . '","'.$_SERVER['REMOTE_ADDR'].'");') or die(mysqli_error($connection));
