...
function vcht_init() {
    ...
    if (!vcht_isIframe() || window.parent.document.location.href.indexOf('console') < 0) {
        ...
        jQuery('body').append('<iframe id="vcht_chatframe" src="' + vcht_url + 'chat-app/include/chatApp_content.php" class="' + vcht_position + '"></iframe>');
    ...
    if (document.location.href.indexOf('vcht_element') > 0) {
        ...
        jQuery.ajax({
            url: wtmt_root+'chat-app/include/clientAjax.php',
            type: 'POST',
            data: {
...
function vcht_showElement(el, avatarImg) {
    if (jQuery(el).length > 0) {
        if (jQuery('.vcht_avatarSel').length > 0) {
            ...
            if (avatarImg) {
                vcht_avatarSel = jQuery('<div class="vcht_avatarSel" style="background-image:  url(\'' + vcht_url + 'chat-app/console/include/files/' + avatarImg + '\');"><div class="vcht_avatarArrow"></div></div>');
            } else {
